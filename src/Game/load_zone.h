...
namespace Load {
	...
	newZone.emplace_or_replace<Component::Dungeon_Level>(newEntity) = oldZone.get<Component::Dungeon_Level>(oldEntity);
    }
	...
	for (auto playerID: view) {
	    ...
	    Copy_Stats(newZone, oldZone, newEntity, playerID);
	    Skill_Component::Copy_Skills(newZone, oldZone, newEntity, playerID);
