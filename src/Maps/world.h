...
//    The first time the tile is rendered generate objects but only the first time
    ...
    //position of tile
    void Generate_Trees(SDL_FRect rect) {
        Procedural_Components::Seed seed;
//    collisionBox;
        Collision::aabb aabb;
...
//        if (chance > 50) {
        for (int k = 0; k < numObjects; k++) {
            ...
            xmlIndex = Procedural_Generation::Random_Int(1, Game_Objects_Lists::forestObjectVec.size() - 1, seed);
            std::string unitName = Game_Objects_Lists::forestObjectVec[xmlIndex];
            std::string imgPath = "";
            //Create_Entities::Create_Entity(World::zone, rect.x + x, rect.y + y, unitName, "",  false, imgPath, false);
            ii++;
        }
    }
...
//                if entities created do nothing
                    if (tilesToRender[i][j]) {
                    }
//                else create them
                    else {
                        Utilities::Log("creating tile objects");
                        tilesToRender[i][j] = true;
                        Generate_Trees(rect);
                    }
