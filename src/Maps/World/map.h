...
namespace Maps {
    ...
    void Init_Tiles(int &instance, std::string &tilesetName) {
        ...
        if (!data.tilesetXML.empty()) {
            ...
            World::world[instance].sheetData = Texture_Packer::TexturePacker_Import_Tileset(spritesheet, data.name,
                                                                                            data.name,
                                                                                            data.tilesetXML.c_str());
        }
        ...
        for (int i = 0; i < World_Data::REGION_SIZE; ++i) {
            for (int j = 0; j < World_Data::REGION_SIZE; ++j) {
                ...
                if (numObjects < 5) {
                    ...
                    Create_Entities::Create_Entity(zone, state, (i * World::size.width) + x,
                                                   (j * World::size.height) + y, data, false, summon, unitIndex);
                    numUnits++;
                }
            }
        }
        ...
        for (int i = 0; i < World_Data::REGION_SIZE; ++i) {
            for (int j = 0; j < World_Data::REGION_SIZE; ++j) {
                ...
                if (numObjects < avgPerRegion) {
                    ...
                    xmlIndex = Procedural_Generation::Random_Int(0, (int) Game_Objects_Lists::tilesets[tileSet].size(),
                                                                 seed);
                    objectName = Game_Objects_Lists::tilesets[tileSet][xmlIndex];

                    Create_Entities::PVG_Building(zone, state, (i * World::size.width) + x,
                                                  (j * World::size.height) + y, i, j, objectName, xmlIndex);
                }
            }
        }
    }
    ...
    void
    Generate_Trees(entt::registry &zone, int &state, SDL_FRect rect, std::string &tileSet, World_Data::Tile &tile) {
        Procedural_Components::Seed seed;
        ...
        for (int k = 0; k < numObjects; k++) {
            ...
            if (Game_Objects_Lists::tilesets[tileSet].empty()) { return; }
            int xmlIndex = Procedural_Generation::Random_Int(0, (int) Game_Objects_Lists::tilesets[tileSet].size(),
                                                             seed);
            ...
            if (j < World_Data::REGION_SIZE - 1) {
                tile.objects[k].entity = (Create_Entities::PVG_Building(zone, state, rect.x + x, rect.y + y, i, j,
                                                                        objectName, xmlIndex));
            }
        }
    }

    void Generate_Labyrinth_Objects(entt::registry &zone, int &state, SDL_FRect rect, std::string &tileSet,
    ...
    void Render(entt::registry &zone, int &state, Component::Camera &camera) {
        ...
        for (int i = x;
             i < (x + ((camera.screen.w + World::size.width + World::size.width) / World::size.width)); i++) {
            for (int j = y;
                 j < (y + ((camera.screen.h + World::size.height + World::size.height) / World::size.height)); j++) {
                if (i < 0 || j < 0) {
                    continue;
                } else {
                    if (i >= World_Data::REGION_SIZE || j >= World_Data::REGION_SIZE) {
                        ...
                        if (!World_Data::tilesEntities[0][i][j].created) {
                            if (Texture_Data::Packer_Textures.contains(World::world[state].tileset)) {
                                Generate_Labyrinth_Objects(zone, state, rect, World::world[state].tileset,
                                                           World_Data::tilesEntities[0][i][j]);
                                World_Data::tilesEntities[0][i][j].tileObject = (Create_Entities::Create_Tile(zone,
                                                                                                              state, i,
                                                                                                              j,
                                                                                                              "labyrinth0_" +
                                                                                                              std::to_string(
                                                                                                                      World_Data::tilesEntities[0][i][j].tileTexture)));
                                World_Data::tilesEntities[0][i][j].isTileObject = true;
                            } else {
                                Generate_Trees(zone, state, rect, World::world[state].tileset,
                                               World_Data::tilesEntities[0][i][j]);
                            }
                        ...
                        if (Texture_Data::Packer_Textures.contains(World::world[state].tileset)) {
                            auto clipRect = Texture_Data::Packer_Textures[World::world[state].tileset].frameList[
                                    Texture_Data::Packer_Textures[World::world[state].tileset].frameList.size() -
                                    1]
                                    .clip;
