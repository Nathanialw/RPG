...
namespace Lighting {
	...
	for (auto entity: view) {
	    ...
	    Graphics::Set_BlendMode(lightRadiusTexture[light.lightRadius], Graphics::BlendMode::MIN);
	    Graphics::Set_BlendMode(texture, Graphics::BlendMode::MIN);
	...
	for (auto entity: view) {
	    ...
	    SDL_FRect visionRect = {0.0f, 0.0f, camera.screen.w, camera.screen.h};
	    ...
	    SDL_SetRenderTarget(Graphics::renderer, texture);
	    Graphics::Set_BlendMode(LoSTexture, Graphics::BlendMode::MAX);
	...
	Graphics::Set_BlendMode(texture);
    }

    void Update(entt::registry &zone) {
	auto view = zone.view<Component::Position, Component::Camera, Component::Renderable>();
	for (auto entity: view) {
