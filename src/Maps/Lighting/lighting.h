...
namespace Lighting {
    ...
    void Create_Light_radius(int lightStr) {
        ...
        radii[lightStr] = SDL_CreateTexture(Graphics::renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, width, height);
        Set_BlendMode(radii[lightStr]);
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                if (distance <= 1.0f)
                    lightRadiusPixels[index] = (minDarkness << 24) | (red << 16) | (green << 8) | blue;
                else if (distance > (lightRadiusF * 0.5f) * (lightRadiusF * 0.5f))
                    lightRadiusPixels[index] = (maxDarkness << 24) | (red << 16) | (green << 8) | blue;
                else {
...
//                    lightRadiusPixels[index] = (minDarkness << 24) | (red << 16) | (green << 8) | blue;
        ...
        Create_Texture(lightRadiusPixels, radii[lightStr]);
    }
	...
	void Get_Emitters(entt::registry &zone, Component::Camera &camera) {
		...
		for (auto entity : view) {
			...
			Draw_Light_Radius(screenPosition.x, screenPosition.y, light.lightRadius, radii[light.lightRadius]);
		}
	}
