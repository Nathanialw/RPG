...
//    }
	...
	for (int i = 0; i < menu.buttons.size(); i++) {
	    ...
	    SDL_RenderCopyF(Graphics::renderer, menu.buttons[i].textTexture, nullptr, &menu.buttons[i].scaledSize);
	}
    }
	...
	while (SDL_PollEvent(&Events::event)) {
		...
		case SDL_QUIT: {
			...
			SDL_FreeSurface(button.textSurface);
		    }
		    return 1;
		}
		...
		case SDL_KEYDOWN: {
		    ...
		    if (Events::event.key.keysym.sym == SDLK_RETURN) {
			return menu.selected;
		    }

		    if (Events::event.key.keysym.sym == SDLK_RETURN2) {
			return menu.selected;
		    }

		    if (Events::event.key.keysym.sym == SDLK_UP) {
			menu.selected--;
			if (menu.selected < 0)
			    menu.selected = 0;
		    }

		    if (Events::event.key.keysym.sym == SDLK_DOWN) {
			menu.selected++;
			if (menu.selected > menu.buttons.size() - 1)
			    menu.selected = menu.buttons.size() - 1;
		    }
