...
namespace UI_Frames {
  ...
  void Init_Button_Menu() {
    topFrame.background.frame = { 512.0f, 0.0f, 256.0f, 32.0f };
    topFrame.background.texture = Graphics::tooltipBackground;    
  }
  ...
  void Build_Buttons(f2 scale) {
    ...
    for (int i = 0; i < text.size(); i++) {
      topFrame.buttons[i].backgroundTexture = Graphics::default_icon;
	...
	topFrame.background.frame.h,
      ...
      //      Unit_Frames::Update_Frame_Text(scale, topFrame.buttons[i], topFrame.buttons[i].text, {155,100,200});      
  ...
  void Show_Menu_Frame(entt::registry &zone, Component::Camera &camera) {
    ...
    SDL_RenderCopyF(Graphics::renderer, topFrame.background.texture, NULL, &topFrame.background.frame);
    
    for (auto &button : topFrame.buttons) {
      SDL_RenderCopyF(Graphics::renderer, button.backgroundTexture, NULL, &button.frame);
      SDL_RenderCopyF(Graphics::renderer, button.textTexture, NULL, &button.frame);
