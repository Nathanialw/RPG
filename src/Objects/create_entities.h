		...
		if (data.body_type == 1) {
			...
			if (!player) {
				...
				if (!Social_Control::Summon(zone, entity, summon, blendType)) {
					Social_Control::Entity(zone, entity, data.race);
					zone.emplace_or_replace<Component::Location>(entity, position, position);
				}
			...
			std::cout << data.equip_type << ": " << data.temp_type_name << std::endl;
			if (data.temp_type_name == "NPC") {
