...
namespace Collision {
    ...
    void Create_Static_Body_Rect(entt::registry &zone, const int &state, const entt::entity &entity, const float &x,
                                 const float &y, const Collision_Component::aabb aabb) {
    ...
    void Create_Static_Body_Polygon(entt::registry &zone, const int &state, const entt::entity &entity, const float &x,
                                    const float &y, const std::vector<std::vector<tmx::Vector2<float>>> &pointVecs) {
...
#ifdef _WIN32
                b2Vec2 *vertices = new b2Vec2[pointVec.size()];
#else
    ...
    void Create_Kinematic_Body(entt::registry &zone, entt::entity &entity, Component::Body body, float &x, float &y,
                               float &radius, float &mass, bool &isDynamicBody) {
    ...
    void Create_Dynamic_Body(entt::registry &zone, int &state, entt::entity &entity, float &x, float &y, float radius,
                             float mass, bool &isDynamicBody) {
        b2BodyDef bodyDef;
    ...
    void Set_Collision_Box(entt::registry &zone, int &state, entt::entity &entity, std::string &colliderType,
                           Component::Position &position, Collision_Component::aabb &aabb,
                           std::vector<std::vector<tmx::Vector2<float>>> &pointVecs, Component::Line_Segment &line,
                           float &radius) {
        if (colliderType == "polygon") {
            ...
            Collision::Create_Static_Body_Rect(zone, state, entity, position.x, position.y,
    ...
    void Attach_Components(entt::registry &zone, int &state, entt::entity &entity,
                           Collision_Component::Collider_Data &colliderData) {
        if (Collision_Component::polygonColliders.contains(colliderData.name)) {
            ...
            Set_Collision_Box(zone, state, entity, colliderData.colliderType, colliderData.position, colliderData.aabb,
                              colliderData.pointVecs, colliderData.line, colliderData.radius);
        }
    ...
    void Save_Line_Segment(entt::registry &zone, const entt::entity &item, std::string name) {
        if (Collision_Component::polygonColliders.contains(name)) {
            ...
            if (!ff.empty()) {
                ...
                for (auto line: ff) {
                    Component::Line_Segment lineSegment = {{position.x + line.start.x, position.y + line.start.y},
                                                           {position.x + line.end.x,   position.y + line.end.y}};
