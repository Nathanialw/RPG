...
namespace Utilities {
    ...
    SDL_FRect Maintain_Scale_Rect(const SDL_FRect &clippedSprite, const float &scale) {
	SDL_FRect fScaledImage = clippedSprite;
	fScaledImage = {
		fScaledImage.x ,
		fScaledImage.y ,
    ...
    SDL_FRect Center_Rect(const SDL_FRect &rect) {
	return {rect.x - (rect.w * 0.5f), rect.y - (rect.h * 0.5f), rect.w, rect.h};
    }

    SDL_Rect Center_Rect(const SDL_Rect &rect) {
	return {(int) (rect.x - (rect.w * 0.5f)), (int) (rect.y - (rect.h * 0.5f)), (int) rect.w, (int) rect.h};
    }
