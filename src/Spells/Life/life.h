...
namespace Life {
  ...
  int Cast(entt::registry &zone, int &state, entt::entity &caster_ID, Component::Position &position, Component::Direction &direction, Component::Casting &casting, float &targetX, float &targetY) {
    ...
    if (targetData.b) {
      if (zone.get<Component::Entity_Type>(targetData.entity_ID) == Component::Entity_Type::unit) {
        if (zone.get<Component::Alive>(targetData.entity_ID).bIsAlive) {
  ...
  int Heal_Other(entt::registry &zone, int &state, entt::entity &entity, Action_Component::Action &action, int &index, float &x, float &y, std::string objectName) {
    ...
    zone.emplace_or_replace<Component::Casting>(entity, castTime, castTime, x, y, nullptr, "wings", "30_InDispersal4Step3");
    return 0;
  }
}// namespace Life